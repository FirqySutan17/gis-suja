...
<div class="main-content pre-posttest">
	...
	<form class="form-horizontal" action="<?= admin_url('gis/kemitraan') ?>" method="POST" style="margin-bottom: 20px">
        <div class="row" style="padding: 0px 10px; border-bottom: 2px solid #000; padding-bottom: 8px;margin: 0px 0px;  ">
					<div class="col-md-5 col-sm-12 filter-style"  style="display: flex;">
						...
						<select id="area" class="form-control class-select" style="width: 100%;" name="area" required>
								<option value="*" <?= $filter['area'] == '*' ? 'selected' : ''  ?>>- ALL AREA -</option>
								<?php foreach ($area as $i => $v): ?>
										<option <?= $filter['area'] == $i ? 'selected' : '' ?>  value="<?= $i ?>"><?= $i.' - '.$v ?></option>
								<?php endforeach ?>
						</select>
					</div>
	...
	<div class="table-responsive table-container">
		...
		<table class="table table-bordered table-hover" id="example1">
          ...
          <th>ACTION</th>
			...
			<tbody>
				<?php $no = 1; ?>
				<?php foreach ($gisite as $site): ?>
					<tr>
						<td><?= $no++ ?></td>
						<td><?= $site['FARM'].' - '.$site['FARM_NAME'] ?></td>
						<td><?= $site['PLAZMA'].' - '.$site['PLAZMA_NAME'] ?></td>
						<td><?= $site['UNIT'] ?></td>
						<td><?= !empty($area[$site['AREA']]) ? $area[$site['AREA']] : 'N/A'; ?></td>
						<td><?= $site['POPULASI'] ?></td>
						<td><a href="<?= $site['LINK_GMAPS'] ?>" target="_blank"><?= $site['ADDRESS'] ?></a></td>
						<td>
							<a href="<?= base_url('dashboard/gis/kemitraan/edit/' . $site['ID']) ?>" class="btn btn-sm btn-show-detail">EDIT</a>
							<a href="<?= base_url('dashboard/gis/kemitraan/delete/' . $site['ID']) ?>" class="btn btn-sm btn-show-cancel" onclick="return confirm('Yakin ingin menghapus data ini?')">DELETE</a>
						</td>
					</tr>
				<?php endforeach; ?>  
			</tbody>
